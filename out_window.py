...
class Ui_OutputDialog(QDialog):
    ...
    def face_rec_(self, frame, encode_list_known, class_names):
        ...
        def mark_attendance(name):
            ...
            if self.CheckInButton.isChecked():
                ...
                with open('Willard.csv', 'a') as f:
                        if (name != 'unknown'):
                            ...
                            if buttonReply == QMessageBox.Yes:
                                ...
                                self.HoursLabel.setText('Estimating Time...')
                ...
                with open('Willard.csv', 'a') as f:
                        if (name != 'unknown'):
                            buttonReply = QMessageBox.question(self, '' + name, 'Are you Checking Out?',
                                                              QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                            if buttonReply == QMessageBox.Yes:
                                date_time_string = datetime.datetime.now().strftime("%y/%m/%d %H:%M:%S")
    ...
    def ElapseList(self,name):
        with open('Willard.csv', "r") as csv_file:
                    ...
                    if field in row:
                        if field == 'Check In':
                            if row[0] == name:
                                ...
                                Time1 = (datetime.datetime.now().strptime(row[1], '%y/%m/%D %H:%M:%S'))
                                self.TimeList1.append(Time1)
                        if field == 'Check Out':
                            if row[0] == name:
                                #print(f'\t ROW 0 {row[0]}  ROW 1 {row[1]} ROW2 {row[2]}.')
                                Time2 = (datetime.datetime.now().strptime(row[1], '%y/%m/%D %H:%M:%S'))
